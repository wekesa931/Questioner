...
class TestUser(TestApplication):
    ...
    def test_add_user_successful(self):
        ...
        response = self.client.post('/api/v2/user/auth/signup', json=self.users_register, content_type='application/json')
        self.assertIn(u'john', response.data.decode())
        self.assertEqual(response.status_code, 201)

    def test_if_no_user_info_provided(self):
        """ Test if user information is previded """
        users={}
        response = self.client.post('/api/v2/user/auth/signup', json=users, content_type='application/json')
    ...
    def test_if_email_is_taken(self):
        ...
        self.client.post('/api/v2/user/auth/signup', json=self.users, content_type='application/json')
        response = self.client.post('/api/v2/user/auth/signup', json=self.users_three, content_type='application/json')
        self.assertEqual(response.status_code, 400)
        self.assertIn(u'email taken', response.data.decode())

    def test_if_email_is_valid(self):
        """ Test if email is valid """
        response = self.client.post('/api/v2/user/auth/signup', json=self.users_five, content_type='application/json')
        self.assertEqual(response.status_code, 400)
